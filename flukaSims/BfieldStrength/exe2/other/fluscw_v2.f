...
************************************************************************
      ...
      OUTDIR  = '/nfs_scratch/ralharth/mucol-targetstudy/flukaData/BfieldStrength/test/'
      FILENAME = OUTDIR // TRIM(RUNTIT) // "_prod.txt"
     ...
     &        "x[cm]", "y[cm]", "z[cm]",
     &        "cx[-]", "cy[-]", "cz[-]", "T[s]",
     &        "Gen", "Evnt", "Mother", "TrkId", "MId", "PrdTyp"
      END IF
 9999 FORMAT (A6,9A15,6A7)
...
*     !&       (JSCRNG .EQ. 5) .OR. (JSCRNG .EQ. 6) ) ) THEN
     ...
     &        ATRACK, NUCGEN+1, EVNT, JTRACK, JTRKID, MTRKID, PRODTYP
      
      END IF
 7777 FORMAT (I6,9E15.7,6I7)
     ...
     &        ATRACK, NUCGEN, EVNT, PARENT_ID, JTRKID, MTRKID, PRODTYP
         END IF
