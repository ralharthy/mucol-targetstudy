import awkward as ak
import pandas as pd
import numpy as np
import mucol as mu
import pathlib
import config
import os
import sys

# Get B-field from run.py
if len(sys.argv) < 1:
    raise ValueError("Missing Bfield argument")
Bfield = f"{int(sys.argv[1]):02d}"

locations = ["esc", "prod", "det1", "det2", "det3", "det4"]
cols = [
    "id", "energy", "p",
    "x", "y", "z",
    "cx", "cy", "cz",
    "time", "gen", "event", 
    "Mother", "ICODE"
]
meta = {
    "beamEnergy": 8,
    "nPrimaries": 100_000 * config.spawn,
    "Bfield": int(Bfield),
    "target": {
        "material": "Inconel",
        "radius_cm": 0.15,
        "length_cm": 17,
    }
}

# -----------------------
# Main: load everything
# -----------------------
all_events = {}

for loc in locations:
    location_events = []

    if config.spawn > 1:
        for r in range(1, config.spawn + 1):
            runNumber = f"{Bfield}_{r:02d}001"
            filename = f"{config.nfs_scratch_simOutput}/run{runNumber}_{loc}.txt"
            df = pd.read_csv(
                filename,
                sep=r"\s+",
                names=cols,
                skiprows=1
            )
            events = mu.build_events(df)
            events = ak.with_field(events, r, "run")
            location_events.append(events)
            
        # combine events
        events = ak.concatenate(location_events)
        events = ak.with_field(events, loc, "location")
        all_events[loc] = events

    else:
        runNumber = f"{Bfield}001"
        filename = f"{config.nfs_scratch_simOutput}/run{runNumber}_{loc}.txt"
        df = pd.read_csv(
            filename,
            sep=r"\s+",
            names=cols,
            skiprows=1
        )
        events = mu.build_events(df)
        events = ak.with_field(events, 1, "run")
        location_events.append(events)
            
        # combine events
        events = ak.concatenate(location_events)
        events = ak.with_field(events, loc, "location")
        all_events[loc] = events

# -----------------------
# Final structure
# -----------------------
all_events = ak.Record(all_events)
particle_events = mu.combine_particles(all_events, locations)
data = ak.with_field(particle_events, meta, "metadata")

# Output to parquet
# config.nfs_scratch_simData.mkdir(parents=True, exist_ok=True)
output = config.nfs_scratch_simData / pathlib.Path(f"{config.simName}_B{Bfield}.parquet")
ak.to_parquet(data, output)

print(f"\nSaved output to: {output}")