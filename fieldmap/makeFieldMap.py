...
# Config
filename = 'fmcylinder.txt'
coords = fm.getCoords(filename)

print(f'Processing file: {filename} with coordinates: {coords}')
data = fm.dataOrganizer(filename)
data_sorted = fm.sortData(data)

os.makedirs("BfieldDatasets", exist_ok=True)
output = "BfieldDatasets/" + filename.replace('fm', 'fluka').replace('.txt', '.inp')
...
plot_output = "BfieldPlots/" + filename.replace('fm', 'fluka').replace('.txt', '.png')
